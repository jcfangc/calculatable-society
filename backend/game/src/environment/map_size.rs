...
impl MapSize {
    ...
    /// 如果宽度或高度未提供，则使用默认值。
    pub(crate) fn new(width: Option<usize>, height: Option<usize>) -> Self {
        Self {
    ...
    /// 从元组创建一个新的 `MapSize` 实例
    /// - 元组第一个元素为高度，第二个元素为宽度
    pub(crate) fn from_tuple(tuple: (usize, usize)) -> Self {
        Self {
            height: tuple.0,
            width: tuple.1,
        }
